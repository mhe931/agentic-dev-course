...
Worth knowing: when neither file exists, `/init` creates `AGENTS.md` by default — in Exercise 1.1 you overrode that with an explicit argument. That default tells you where the tooling is heading. If your team is Copilot-only, `copilot-instructions.md` is enough. If multiple agent tools are in play (or might be), `AGENTS.md` is the better home for shared conventions — keep any Copilot-specific guidance in `copilot-instructions.md` and avoid duplicating content between the two, since VS Code combines all matching instruction files into context.
